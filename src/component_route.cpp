...
namespace weavess {
    ...
    void ComponentSearchRouteLinearScan::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
                                                std::vector<unsigned int> &res) {
        const auto L = index->getParam().get<unsigned>("L_search");
        const auto K = index->getParam().get<unsigned>("K_search");
        
        pool.resize(L);
        for (unsigned i = 0; (i < L) && (i < index->getBaseLen()); i++) {
            unsigned id = i;
    ...
    void ComponentSearchRouteGreedyF::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
                                                std::vector<unsigned int> &res) {
        const auto L = index->getParam().get<unsigned>("L_search");
        const auto K = index->getParam().get<unsigned>("K_search");

        std::vector<char> flags(index->getBaseLen(), 0); // TODO why not bool?
        ...
        while (k < (int) L) {
            ...
            if (pool[k].flag) {
                pool[k].flag = false;
                unsigned id_curr = pool[k].id; // current node id
                unsigned id_src = pool[k].id_src; // inedge src node id

                index->addHopCount();

                if (id_curr != id_src) {
                    // current node has inedge
                    for (unsigned m = 0; m < index->getInedgeCandidates()[id_curr][id_src].size(); ++m) {

                        unsigned id = index->getInedgeCandidates()[id_curr][id_src][m];
                        ...
                        if (dist >= pool[L - 1].distance) continue;
                        Index::Neighbor nn(id, dist, true, id_curr);
                        int r = Index::InsertIntoPool(pool.data(), L, nn);
                    ...
                    for (unsigned m = 0; m < index->getLoadGraph()[id_curr].size(); ++m) {
                        unsigned id = index->getLoadGraph()[id_curr][m];
