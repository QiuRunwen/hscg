...
namespace weavess {
    ...
    void ComponentConnNSGSphere2DFS::ConnInner() {
        tree_grow(index->eps_[0]);
        tree_grow(index->eps_[1]);
    }

    void ComponentConnNSGSphere2DFS::tree_grow(unsigned root) {
        boost::dynamic_bitset<> flags{index->getBaseLen(), 0};
        unsigned unlinked_cnt = 0;
        while (unlinked_cnt < index->getBaseLen()) {
            DFS(flags, root, unlinked_cnt);
    ...
    void ComponentConnNSGSphere2DFS::DFS(boost::dynamic_bitset<> &flag, unsigned root, unsigned &cnt) {
        unsigned tmp = root;
        std::stack<unsigned> s;
        s.push(root);
        if (!flag[root]) cnt++;
    ...
    void ComponentConnNSGSphere2DFS::findroot(boost::dynamic_bitset<> &flag, unsigned &root) {
        unsigned id = index->getBaseLen();
        for (unsigned i = 0; i < index->getBaseLen(); i++) {
            if (flag[i] == false) {
                id = i;
    ...
    void ComponentConnNSGSphere2DFS::get_neighbors(const float *query, std::vector<Index::Neighbor> &retset,
                                            std::vector<Index::Neighbor> &fullset) {
        unsigned L = index->getParam().get<unsigned>("L_refine");
        ...
        flags[index->eps_[0]] = true;
        init_ids[1] = index->eps_[1];
        flags[index->eps_[1]] = true;
        L = 2;
