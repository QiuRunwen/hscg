...
#pragma omp for schedule(dynamic, 100)
        ...
        std::cout << "ComponentRefineNNDescent Time Cost: " << std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    void ComponentRefineNNDescent::NNDescent() {
        if (index->debug == true) {
            ...
            for (unsigned it = 0; it < index->ITER; it++) {
                ...
                std::cout << "CurrentIter TimeCost: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::steady_clock::now() - begin).count() / 1000.0 << std::endl;
...
#pragma omp parallel for
        ...
        for (unsigned n = 0; n < index->getBaseLen(); ++n) {
            ...
            for (unsigned l = 0; l < nnhd.M; ++l) {
                ...
                if (nn.flag) {
                    // if (rand() % 2)continue; // randomly skip a few edges
...
#pragma omp for schedule(dynamic, 100)
    ...
    /**
    * NSG Refine :
    *  Entry     : CentroidSphere2
    *  CANDIDATE : GREEDY(NSGSphere2)
    *  PRUNE     : NSG
    *  CONN      : Sphere2DFS
    */
   void ComponentRefineNSGSphere2::RefineInner() {

        SetConfigs();

        // ENTRY
        std::cout << "__ENTRY : CentroidSphere2__" << std::endl;
        auto *a = new ComponentRefineEntryCentroidSphere2(index);
        a->EntryInner();
        std::cout << "__ENTRY : FINISH" << std::endl;
        ...
        // CONN
        std::cout << "__CONN : Sphere2DFS__" << std::endl;
        auto *c = new ComponentConnNSGSphere2DFS(index);

        c->ConnInner();
    }

    void ComponentRefineNSGSphere2::SetConfigs() {
        index->R_refine = index->getParam().get<unsigned>("R_refine");
        index->L_refine = index->getParam().get<unsigned>("L_refine");
        index->C_refine = index->getParam().get<unsigned>("C_refine");
    ...
    void ComponentRefineNSGSphere2::Link(Index::SimpleNeighbor *cut_graph_) {
        std::vector<std::mutex> locks(index->getBaseLen());

        // CANDIDATE
        std::cout << "__CANDIDATE : GREEDY(NSGSphere2)__" << std::endl;
        auto *a = new ComponentCandidateNSGSphere2(index);
...
#pragma omp for schedule(dynamic, 100)
    ...
    void ComponentRefineNSGSphere2::InterInsert(unsigned n, unsigned range, std::vector<std::mutex> &locks,
                                        Index::SimpleNeighbor *cut_graph_) {
        Index::SimpleNeighbor *src_pool = cut_graph_ + (size_t) n * (size_t) range;
        for (size_t i = 0; i < range; i++) {
            if (src_pool[i].distance == -1) break;
    ...
    /**NSG_LSHCS Refine :
     * Entry     : Centroid
     * CANDIDATE : GREEDY(NSG) or PROPAGATION 2 or LSHCS
     * PRUNE     : NSG
     * CONN      : DFS
     */
    void ComponentRefineNSG_LSHCS::RefineInner() {
        ...
        // CONN
        std::cout << "__CONN : DFS__" << std::endl;
        auto *c = new weavess::ComponentConnNSGDFS(index);

        c->ConnInner();
    }

    void ComponentRefineNSG_LSHCS::SetConfigs() {
        index->R_refine = index->getParam().get<unsigned>("R_refine");
        index->L_refine = index->getParam().get<unsigned>("L_refine");
        index->C_refine = index->getParam().get<unsigned>("C_refine");
    ...
    void ComponentRefineNSG_LSHCS::Link(Index::SimpleNeighbor *cut_graph_) {
        std::vector<std::mutex> locks(index->getBaseLen());
        ...
        // std::cout << "__CANDIDATE : GREEDY(LSHCS)__" << std::endl;
        ComponentCandidate *a = new ComponentCandidateLSHCS(index);

        // PRUNE
        std::cout << "__PRUNE : NSG__" << std::endl;
        ComponentPrune *b = new ComponentPruneNSG(index);
...
#pragma omp for schedule(dynamic, 100)
    ...
    void ComponentRefineNSG_LSHCS::InterInsert(unsigned n, unsigned range, std::vector<std::mutex> &locks,
                                         Index::SimpleNeighbor *cut_graph_) {
        Index::SimpleNeighbor *src_pool = cut_graph_ + (size_t) n * (size_t) range;
        for (size_t i = 0; i < range; i++) {
            if (src_pool[i].distance == -1) break;
    ...
    *  CONN      : DFS
    */
    void ComponentRefineNSG_F::RefineInner() {
        ...
        // filter list for each in-edge of each node
        index->getInedgeCandidates().resize(index->getBaseLen());
        for (size_t inedge_start_id = 0; inedge_start_id < index->getBaseLen(); inedge_start_id++) {
            for (auto& curr_n : index->getFinalGraph()[inedge_start_id]) {

                for (auto& outedge_end : index->getFinalGraph()[curr_n.id]) {
                    if (index->getDist()->calc_angle_if_lt_90(
                        index->getBaseData() + index->getBaseDim() * inedge_start_id,
                        index->getBaseData() + index->getBaseDim() * curr_n.id,
                        index->getBaseDim())) 
                    {
                        index->getInedgeCandidates()[curr_n.id][inedge_start_id].push_back(outedge_end.id);
                    }
                }
            }
        }
        std::cout << "getInedgeCandidates : FINISH__" << std::endl;
    }

    void ComponentRefineNSG_F::SetConfigs() {
        index->R_refine = index->getParam().get<unsigned>("R_refine");
        index->L_refine = index->getParam().get<unsigned>("L_refine");
        index->C_refine = index->getParam().get<unsigned>("C_refine");
    ...
    void ComponentRefineNSG_F::Link(Index::SimpleNeighbor *cut_graph_) {
        std::vector<std::mutex> locks(index->getBaseLen());
...
#pragma omp for schedule(dynamic, 100)
    ...
    void ComponentRefineNSG_F::InterInsert(unsigned n, unsigned range, std::vector<std::mutex> &locks,
                                         Index::SimpleNeighbor *cut_graph_) {
        Index::SimpleNeighbor *src_pool = cut_graph_ + (size_t) n * (size_t) range;
        for (size_t i = 0; i < range; i++) {
            if (src_pool[i].distance == -1) break;
