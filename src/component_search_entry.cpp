...
namespace weavess {
    ...
    void ComponentSearchEntryCentroidSphere2::SearchEntryInner(unsigned int query, std::vector<Index::Neighbor> &pool) {
        const auto L = index->getParam().get<unsigned>("L_search");
        pool.reserve(L + 1);
        std::vector<unsigned> init_ids(L);
        boost::dynamic_bitset<> flags{index->getBaseLen(), 0};
        ...
        // index->getFinalGraph() is not initialized in `search`
        // it will casue segment fault
