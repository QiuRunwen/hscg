...
namespace weavess {
    ...
    /*Read .fvecs and .ivecs files. each line is `n_dim, v1, v2, ..., vn_dim` (No commas and spaces)  */
    ...
    /*Read .ibin or .fbin files. First line is `nrow, ndim` . each line is `v1, v2, ..., v_ndim` (No commas and spaces) */
    inline void load_data(char *filename, T *&data, unsigned &num, unsigned &dim) {
        std::ifstream in(filename, std::ios::binary);
        if (!in.is_open()) {
            std::cerr << "open file error" << std::endl;
