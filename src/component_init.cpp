...
#pragma omp parallel for
    ...
    void ComponentInitLSHCS::InitInner() {
        SetConfigs();

        if (index->getParam().get<bool>("UseColliCount")) {
            InitInnerColliCount();
        } else {
            InitInnerNaive();
        }
    ...
    void ComponentInitLSHCS::InitInnerNaive() {

        const float lshedge_ratio = 1.0f;
        unsigned n_edge_4_lsh = (unsigned) (index->getInitEdgesNum() * lshedge_ratio);
        // unsigned n_edge_4_lsh = index->getInitEdgesNum();
        unsigned hash_l = index->getParam().get<unsigned>("hash_l");
        unsigned hash_t = index->getParam().get<unsigned>("hash_t");
        unsigned base_len = index->getBaseLen();
        unsigned base_dim = index->getBaseDim();

        index->getFinalGraph().resize(base_len);
        unsigned hashval_range = 1 << hash_t; // 2^hash_t. hash_t should be less than 32.

        Eigen::Matrix<unsigned, Eigen::Dynamic, Eigen::Dynamic> hash_tables(base_len, hash_l);

        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
        build_hashtables(hash_tables);
        std::cout << "build_hashtables cost: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - begin).count() / 1000.0  << std::endl;
        
        begin = std::chrono::steady_clock::now();
        std::vector<std::unordered_map<unsigned, std::vector<unsigned>>> maps(hash_l);
#ifdef PARALLEL
#pragma omp parallel for
#endif
        for (unsigned j = 0; j < hash_l; j++) {
            for (unsigned i = 0; i < base_len; i++) {
                unsigned hash_val = hash_tables(i, j);
                maps[j][hash_val].push_back(i);
            }
        }
        std::cout << "build maps cost: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - begin).count() / 1000.0 << std::endl;
...
#pragma omp parallel for
#endif
        for (unsigned i = 0; i < base_len; i++) {
            // TODO std::vector<SimpleNeighbor> &pool = index->getFinalGraph()[i];
            index->getFinalGraph()[i].reserve(n_edge_4_lsh);

            std::unordered_set<unsigned> flags;
            for (unsigned j = 0; j < hash_l; j++) {
                unsigned hash_val = hash_tables(i, j);
                for (unsigned id : maps[j][hash_val]) {
                    if (id == i) continue;
                    if (flags.find(id) != flags.end()) continue;
                    flags.insert(id);

                    float dist = index->getDist()->compare(index->getBaseData() + i * base_dim,
                                                           index->getBaseData() + id * base_dim,
                                                           base_dim);
                    index->getFinalGraph()[i].emplace_back(id, dist);
                    if (index->getFinalGraph()[i].size() >= n_edge_4_lsh) {
                        break;
                    }
                }
                if (index->getFinalGraph()[i].size() >= n_edge_4_lsh) {
            ...
            // TODO fill the rest with random
            while (index->getFinalGraph()[i].size() < static_cast<size_t>(index->getInitEdgesNum())) {
                unsigned id = rand() % base_len;
                if (id == i) continue;
                if (flags.find(id) != flags.end()) continue;
                flags.insert(id);
                float dist = index->getDist()->compare(index->getBaseData() + i * base_dim,
        ...
        std::cout << "build graph cost: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - begin).count() / 1000.0 << std::endl;

        // free hash_tables and maps
        hash_tables.resize(0, 0);
        maps.clear();
    }

    
    void ComponentInitLSHCS::InitInnerColliCount() {
        ...
        constexpr unsigned maxEnumD = 2;

        // We'll define a custom signature type that holds a signature as a
        // vector of unsigned values. (Each point’s signature comes from one row
        // of hash_tables.)
        struct SignatureEigen {
            std::vector<unsigned> data;
            // Constructor that allocates L entries.
            SignatureEigen(unsigned L) : data(L) { }
            
            // Equality operator: two signatures are equal if their data vectors are equal.
            bool operator==(const SignatureEigen &other) const {
                return data == other.data;
            }
        };

        // Custom hash functor for SignatureEigen.
        struct SignatureEigenHash {
            std::size_t operator()(const SignatureEigen &s) const {
                std::size_t h = 0;
                for (auto x : s.data) {
                    h = h * 257 + x;  // 257 is prime and helps mix the values.
                }
                return h;
            }
        };

        // ------------------ Helper: Hamming Collision Count ---------------
        // Given a query signature (SignatureEigen) and a point index, count the number
        // of positions in which the signature of point i (from hash_tables) matches the query.
        auto hammingCollisionCount = [&](const SignatureEigen &querySig, unsigned pointIdx) -> unsigned {
            unsigned count = 0;
            for (unsigned j = 0; j < hash_l; j++) {
                if (hash_tables(pointIdx, j) == querySig.data[j])
                    count++;
            }
            return count;
        };
        ...
        // ---------------------- Build the BucketIndex -------------------------
        // We build an index that maps each full signature to a vector of point indices
        // that share that signature.
        std::unordered_map<SignatureEigen, std::vector<unsigned>, SignatureEigenHash> map_bucket_idxs;
        map_bucket_idxs.reserve(base_len);
        
        begin = std::chrono::steady_clock::now();
        for (unsigned i = 0; i < base_len; i++) {
            SignatureEigen sig(hash_l);
            for (unsigned j = 0; j < hash_l; j++) {
                sig.data[j] = hash_tables(i, j);
            }
            map_bucket_idxs[sig].push_back(i);
        }
        std::cout << "build map_bucket_idxs cost: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - begin).count() / 1000.0 << std::endl;

        // ------------------ getTopK Lambda Function -----------------------
        // For a given query point (by index), this lambda returns a vector of pairs:
        // (collision_count, candidate_index) for candidate points, sorted in descending
        // order by collision count.
        auto getTopK = [&](unsigned queryIndex) -> std::vector<std::pair<unsigned,unsigned>> {
            // candMap maps candidate point indices to the best collision count found.
            std::unordered_map<unsigned, unsigned> candMap;
            
            // Build the query signature from the hash_tables.
            SignatureEigen querySig(hash_l);
            for (unsigned j = 0; j < hash_l; j++) {
                querySig.data[j] = hash_tables(queryIndex, j);
            }
            
            // --- Step 1: Exact Match (d = 0) ---
            // warning: init-statement in selection statements only available with ‘-std=c++17’
            // if (auto it = map_bucket_idxs.find(querySig); it != map_bucket_idxs.end()) {
            auto it = map_bucket_idxs.find(querySig);
            if (it != map_bucket_idxs.end()){
                for (unsigned cand : it->second) {
                    if (cand == queryIndex) continue; // skip the query point
                    candMap[cand] = hash_l; // full match → collision count equals L
                }
            }
            
            // --- Step 2: Enumerate Neighbors for d = 1 to maxEnumD ---
            // This recursive lambda enumerates all signatures that differ in exactly
            // d positions from querySig. Each such candidate has collision count L-d.
            // The implement forbids to construct map_bucket_idxs multiple times and reduce overhead.
            // but if hash_t and hash_l are large, the cost of this implementation is high.
            std::function<void(unsigned, unsigned, SignatureEigen&, unsigned)> enumerateNeighbors;
            enumerateNeighbors = [&](unsigned start, unsigned d_remaining, SignatureEigen &current, unsigned d_value) {
                if (candMap.size() >= static_cast<size_t>(n_edge_4_lsh)) {
                    return;
                }

                if (d_remaining == 0) {
                    // current now differs in exactly d_value positions.
                    auto it = map_bucket_idxs.find(current);
                    if (it != map_bucket_idxs.end()) {
                        for (unsigned cand : it->second) {
                            if (cand == queryIndex) continue;
                            candMap[cand] = hash_l - d_value;
                            if (candMap.size() >= static_cast<size_t>(n_edge_4_lsh)) {
                                return;
                            }
                            // unsigned collisionCount = hash_l - d_value;
                            // if (candMap.find(cand) == candMap.end() || candMap[cand] < collisionCount)
                                // candMap[cand] = collisionCount;
                        }
                    }
                    return;
                }
                // Choose positions from 'start' to L - d_remaining.
                for (unsigned pos = start; pos < hash_l - d_remaining + 1; pos++) {
                    unsigned orig = current.data[pos];
                    // Try every alternative value (here we assume values in [0, 255]).
                    for (unsigned a = 0; a < hashval_range; a++) {
                        if (a == orig)
                            continue;
                        current.data[pos] = a;
                        enumerateNeighbors(pos + 1, d_remaining - 1, current, d_value);
                    }
                    // Restore the original value.
                    current.data[pos] = orig;
                }
            };
            
            // Enumerate neighbors for differences d = 1, 2, … up to maxEnumD,
            // stopping early if we already have at least K candidates.
            for (unsigned d = 1; d <= maxEnumD && candMap.size() < static_cast<size_t>(n_edge_4_lsh); d++) {
                SignatureEigen current = querySig; // start with the query signature
                enumerateNeighbors(0, d, current, d);
                if (candMap.size() >= static_cast<size_t>(n_edge_4_lsh))
                    break;
            }
            
            // --- Step 3: Fallback to Linear Scan if Necessary ---
            // std::cout << "candMap.size(): " << candMap.size() << ". Back to Linear Scan." << std::endl;
            // if (candMap.size() < static_cast<size_t>(n_edge)) {
            //     for (unsigned i = 0; i < base_len; i++) {
            //         if (i == queryIndex) continue;
            //         if (candMap.find(i) == candMap.end()) {
            //             unsigned collisionCount = hammingCollisionCount(querySig, i);
            //             candMap[i] = collisionCount;
            //         }
            //     }
            // }

            // --- Step 3 or randomly select some points to fill the result ---
            while (candMap.size() < static_cast<size_t>(index->getInitEdgesNum())) {
                unsigned cand = rand() % base_len;
                if (cand == queryIndex) continue;
                if (candMap.find(cand) == candMap.end()) {
                    // unsigned collisionCount = hammingCollisionCount(querySig, cand);
                    candMap[cand] = 0;
                }
            }
            
            // --- Step 4: Build and Sort the Result ---
            std::vector<std::pair<unsigned,unsigned>> result;
            for (const auto &p : candMap) {
                result.push_back({p.second, p.first});
            }
            std::sort(result.begin(), result.end(),
                    [](const std::pair<unsigned,unsigned>& a, const std::pair<unsigned,unsigned>& b) {
                        return a.first > b.first; // sort descending by collision count
                    });
            if (result.size() > static_cast<size_t>(index->getInitEdgesNum()))
                result.resize(index->getInitEdgesNum());
            return result;
        };
...
#pragma omp parallel for
        ...
        for (unsigned i = 0; i < base_len; i++) {
            ...
            for (const auto &p : topk) {
                float dist = index->getDist()->compare(index->getBaseData() + i * base_dim,
                                                       index->getBaseData() + p.second * base_dim,
                                                       base_dim);
                index->getFinalGraph()[i].emplace_back(p.second, dist);
            }
    ...
    void ComponentInitLSHCS::SetConfigs() {
        index->setInitEdgesNum(index->getParam().get<unsigned>("S"));
    }


    void ComponentInitLSHCS::build_hashtables(Eigen::Matrix<unsigned, Eigen::Dynamic, Eigen::Dynamic> &hash_tables) {
        unsigned hash_l = index->getParam().get<unsigned>("hash_l");

        for (unsigned i = 0 ; i < hash_l; i++) {
            build_one_hashtable(hash_tables.col(i));
        }
    }

    void ComponentInitLSHCS::build_one_hashtable(Eigen::Ref<Eigen::Vector<unsigned, Eigen::Dynamic>> hashtable) {
        ...
        unsigned base_len = index->getBaseLen();
        unsigned base_dim = index->getBaseDim();
        Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> base_data(index->getBaseData(), base_len, base_dim);

        Eigen::MatrixXf normal_distr_m = Eigen::MatrixXf::Zero(base_dim, hash_t).unaryExpr([&](float x) { return normal_distr(e); });
        
        // N*d @ d*t = N*t
        // Eigen::MatrixXf tmp_hash_table = base_data * normal_distr_m;
        // Eigen::MatrixXi matrix_if_gt_0= (tmp_hash_table.array() > 0).cast<int>();

        Eigen::Matrix<unsigned, Eigen::Dynamic, Eigen::Dynamic> matrix_if_gt_0= ((base_data * normal_distr_m).array() > 0).cast<unsigned>();

        if (hash_t > 32) {
            std::cerr << "hash_t should be less than 32" << std::endl;
            exit(-1);
        }
        // convert each row (only 1/0) to unsigned
#ifdef PARALLEL
#pragma omp parallel for
#endif
        for (unsigned i = 0; i < base_len; i++) {
            unsigned hashvalue = 0;
            for (unsigned j = 0; j < hash_t; j++) {
                
                // two ways to calculate hashvalue are the same, |= is more explicit for bitwise encoding
                // hashvalue += matrix_if_gt_0(i, j) << j;
                hashvalue |= matrix_if_gt_0(i, j) << j;
            }
            hashtable(i) = hashvalue;
        }
    ...
    void ComponentInitKNNG::InitInner() {
        ...
        index->getFinalGraph().resize(index->getBaseLen());
        float scan_rate = index->getParam().get<float>("RATIO", 1.0f);
        unsigned scan_num = (unsigned) (index->getBaseLen() * scan_rate);
        scan_num = std::min(scan_num, index->getBaseLen() - 1);

        auto GenRandom = [&](unsigned N, unsigned size, unsigned *addr, unsigned exclude) {
            for (unsigned i = 0; i < size; ++i) {
                addr[i] = rand() % (N - size);
            }
            std::sort(addr, addr + size);
...
#pragma omp parallel for
        ...
        for (unsigned i = 0; i < index->getBaseLen(); i ++) {
            ...
            int pos = 0;
            std::vector<unsigned> indices(scan_num);
            // random select scan_num points

            GenRandom(index->getBaseLen(), scan_num, indices.data(), i);
