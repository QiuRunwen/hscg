...
void LinearScan(weavess::Parameters &parameters){
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        std::cout << "Build cost: 0"  << std::endl;
    }else if (parameters.get<std::string>("exc_type") == "search") {    // search
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                // -> load_graph(weavess::TYPE::INDEX_KGRAPH, &graph_file[0])
                -> search(weavess::TYPE::SEARCH_ENTRY_NONE, weavess::TYPE::ROUTER_LINEARSCAN, weavess::TYPE::L_SEARCH_ASCEND);
...
void KGraph(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_KGRAPH, &graph_file[0])
                -> search(weavess::TYPE::SEARCH_ENTRY_RAND, weavess::TYPE::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_KGRAPH, &graph_file[0])
...
void FANNG(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_RAND, weavess::TYPE::ROUTER_BACKTRACK, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_FANNG, &graph_file[0])
...
void NSG(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_CENTROID, weavess::TYPE::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_NSG, &graph_file[0])
...
void NSG_SPHERE2(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> refine(weavess::REFINE_NN_DESCENT, false);
        std::cout << "Init cost: " << builder->GetBuildTime().count() << std::endl;
        builder -> refine(weavess::REFINE_NSG_SPHERE2, false)
                -> save_graph(weavess::TYPE::INDEX_SSG, &graph_file[0]);
        std::cout << "Build cost: " << builder->GetBuildTime().count() << std::endl;
    }else if (parameters.get<std::string>("exc_type") == "search") {    // search
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_SSG, &graph_file[0])
                -> search(weavess::TYPE::SEARCH_ENTRY_CENTROID_SPHERE2, weavess::TYPE::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_SSG, &graph_file[0])
...
void NSG_LSHCS(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> init(weavess::INIT_LSHCS)
                -> refine(weavess::REFINE_NN_DESCENT, false);
        std::cout << "Init cost: " << builder->GetBuildTime().count() << std::endl;
        builder -> refine(weavess::REFINE_NSG, false)
                -> save_graph(weavess::TYPE::INDEX_NSG_LSHCS, &graph_file[0]);
        std::cout << "Build cost: " << builder->GetBuildTime().count() << std::endl;
    }else if (parameters.get<std::string>("exc_type") == "search") {    // search
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_NSG_LSHCS, &graph_file[0])
                -> search(weavess::TYPE::SEARCH_ENTRY_CENTROID, weavess::TYPE::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_NSG_LSHCS, &graph_file[0])
                -> print_index_info(weavess::TYPE::INDEX_NSG_LSHCS);
    }else {
        std::cout << "exc_type input error!" << std::endl;
    }
}

void NSG_LSHCS_NONNDES(weavess::Parameters &parameters) {
...
void NSG_LSH_SPHERE2(weavess::Parameters &parameters) {
    ...
    // Two centroids (entry point) are used to search the graph.
    // Therefore, save/load the graph as SSG (Multiple entry point + graph structure).
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> init(weavess::INIT_LSHCS)
                -> refine(weavess::REFINE_NN_DESCENT, false);
...
void NSG_KDT(weavess::Parameters &parameters) {
...
void NSG_KNNG(weavess::Parameters &parameters) {
...
void SSG(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_SSG, &graph_file[0])
                -> search(weavess::SEARCH_ENTRY_SUB_CENTROID, weavess::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_SSG, &graph_file[0])
...
void EFANNA(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_KDT, weavess::TYPE::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_EFANNA, &graph_file[0])
...
void NSW(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_NONE, weavess::TYPE::ROUTER_NSW, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_NSW, &graph_file[0])
...
void HNSW(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_NONE, weavess::TYPE::ROUTER_HNSW, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_HNSW, &graph_file[0])
...
void PANNG(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_VPT, weavess::TYPE::ROUTER_NGT, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_PANNG, &graph_file[0])
...
void HCNNG(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_KDT, weavess::TYPE::ROUTER_GUIDE, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_HCNNG, &graph_file[0])
...
void SPTAG_KDT(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_NONE, weavess::TYPE::ROUTER_SPTAG_KDT, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_SPTAG_KDT, &graph_file[0])
...
void SPTAG_BKT(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        ...
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                ...
                -> search(weavess::TYPE::SEARCH_ENTRY_NONE, weavess::TYPE::ROUTER_SPTAG_BKT, weavess::TYPE::L_SEARCH_ASCEND);
        builder -> peak_memory_footprint();
    }else if (parameters.get<std::string>("exc_type") == "info") {
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> load_graph(weavess::TYPE::INDEX_SPTAG_BKT, &graph_file[0])
...
void KDRG(weavess::Parameters &parameters) {
    ...
    if (parameters.get<std::string>("exc_type") == "build") {   // build
        builder -> load(&base_path[0], &query_path[0], &ground_path[0], parameters)
                -> init(weavess::INIT_KNNG, false);
        std::cout << "Init cost: " << builder->GetBuildTime().count() << std::endl;
        builder -> refine(weavess::REFINE_KDRG, false)
...
int main(int argc, char** argv) {
    ...
    std::string dataset_root = R"(//mnt/d/phd/VDB/data/)";
    // std::string dataset_root = R"(//mnt/d/phd/VDB/data/sample/)";
    ...
    parameters.set<unsigned>("n_threads", 32);
    ...
    std::string exact_knng_path("//mnt/d/phd/VDB/data/knn/siftsmall_50nn.graph"); // TODO
    ...
    std::string search_res_path_prefix(alg + "_" + dataset + "_search");
    parameters.set<std::string>("search_res_path_prefix", search_res_path_prefix);
    set_para(alg, dataset, parameters);

    // alg
    if (alg == "linearscan") {
        LinearScan(parameters);
    }else if (alg == "kgraph") {
        KGraph(parameters);
    }else if (alg == "fanng") {
        FANNG(parameters);
        ...
        NSG_LSHCS(parameters);
    }else if (alg == "nsg_lshcs_nonndes") {
        NSG_LSHCS_NONNDES(parameters);
    }else if (alg == "nsg_sphere2") {
        NSG_SPHERE2(parameters);
    }else if (alg == "nsg_lsh_sphere2"){
        NSG_LSH_SPHERE2(parameters);
